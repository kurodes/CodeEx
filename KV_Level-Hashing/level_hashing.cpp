...
uint64_t F_HASH(level_hash *level, const uint8_t *key) {
    return (hash((void *)key, strlen((char *)key), level->f_seed));
}
...
uint64_t S_HASH(level_hash *level, const uint8_t *key) {
    return (hash((void *)key, strlen((char *)key), level->s_seed));
}
...
level_hash *level_init(uint64_t level_size)
{
    ...
    printf("The number of top-level buckets: %llu\n", level->addr_capacity);
    printf("The number of all buckets: %llu\n", level->total_capacity);
    printf("The number of all entries: %llu\n", level->total_capacity*ASSOC_NUM);
...
uint8_t* level_dynamic_query(level_hash *level, uint8_t *key)
{
    ...
    if(level->level_item_num[0] > level->level_item_num[1]){
        ...
        for(i = 0; i < 2; i ++){
            for(j = 0; j < ASSOC_NUM; j ++){
                if (level->buckets[i][f_idx].token[j] == 1&&strcmp((char *)level->buckets[i][f_idx].slot[j].key, (char *)key) == 0)
                {
                    return level->buckets[i][f_idx].slot[j].value;
                }
            }
            for(j = 0; j < ASSOC_NUM; j ++){
                if (level->buckets[i][s_idx].token[j] == 1&&strcmp((char *)level->buckets[i][s_idx].slot[j].key, (char *)key) == 0)
                {
        ...
        for(i = 2; i > 0; i --){
            for(j = 0; j < ASSOC_NUM; j ++){
                if (level->buckets[i-1][f_idx].token[j] == 1&&strcmp((char *)level->buckets[i-1][f_idx].slot[j].key, (char *)key) == 0)
                {
                    return level->buckets[i-1][f_idx].slot[j].value;
                }
            }
            for(j = 0; j < ASSOC_NUM; j ++){
                if (level->buckets[i-1][s_idx].token[j] == 1&&strcmp((char *)level->buckets[i-1][s_idx].slot[j].key, (char *)key) == 0)
                {
